...
// APPROACH 7 TEST: Direct bash WebSocket (bypasses ttyd)
app.get('/test/bash/:sandboxId', async (c) => {
  const upgrade = c.req.header('Upgrade');
  if (upgrade?.toLowerCase() !== 'websocket') {
    return c.text('Expected WebSocket upgrade', 400);
  }
  ...
  console.log('[Worker] APPROACH 7 TEST: Direct bash for sandbox:', sandboxId);

  const tabBroadcast = getTabBroadcast(c.env, sandboxId);
  // URL must end with /bash-ws for TabBroadcastDO to route to handleDirectBash
  const bashWsUrl = new URL(`/terminal/test/${sandboxId}/bash-ws`, c.req.url);
  const bashWsRequest = new Request(bashWsUrl.toString(), {
    method: c.req.raw.method,
    headers: c.req.raw.headers,
  });

  return tabBroadcast.fetch(bashWsRequest);
});
  ...
  // Simple iframe terminal - starts ttyd and returns preview URL
  app.post('/simple/:id/start', async (c) => {
    const sessionId = c.req.param('id');
    const { hostname } = new URL(c.req.url);

    if (startedSessions.has(sessionId)) {
      return c.json({ ready: true, url: previewUrls.get(sessionId) });
    }
    ...
    try {
      ...
      const exposed: any = await sandbox.exposePort(7681, { hostname });
      const url = exposed.url;

      startedSessions.add(sessionId);
      previewUrls.set(sessionId, url);

      return c.json({ ready: true, url });
    } catch (error) {
      return c.json({ error: String(error) }, 500);
    }
  });
