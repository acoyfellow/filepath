  ...
  async run(
    ...
    try {
      ...
      const container = await step.do('create-container', async () => {
        ...
        const sandbox = getSandbox((this.env as Env).Sandbox as unknown as DurableObjectNamespace<Sandbox>, containerId);
      ...
      const wsUrl = await step.do('start-ttyd', async () => {
        ...
        const sandbox = getSandbox((this.env as Env).Sandbox as unknown as DurableObjectNamespace<Sandbox>, container.id);
