...
<div class="min-h-screen bg-[var(--bg2)] font-[var(--f)] text-[var(--t2)] transition-colors duration-200">
  <main class="mx-auto flex w-full max-w-3xl flex-col px-6 py-12 max-[640px]:px-4 max-[640px]:py-8">
    <div class="mb-8 flex items-start justify-between gap-4 max-[640px]:mb-6 max-[640px]:flex-col max-[640px]:items-stretch">
      <div>
        <h1 class="text-2xl font-[650] tracking-[-0.03em] text-[var(--t1)]">Workspaces</h1>
        <p class="mt-1.5 max-w-2xl text-sm leading-6 text-[var(--t4)]">
          Your personal background agents, each scoped to a sandboxed git clone.
        </p>
      </div>
      <Button
        class="h-10 gap-2 rounded-full bg-[var(--accent)] px-4 text-white shadow-none hover:opacity-90 max-[640px]:w-full"
    ...
    {#if isLoading}
      <div class="py-16 text-center">
        <div class="mx-auto mb-3 size-5 animate-spin rounded-full border-2 border-[var(--b1)] border-t-[var(--accent)]"></div>
        <p>Loading workspaces...</p>
      </div>
    {:else if workspaces.length === 0}
      <div class="py-16 text-center">
        <p class="text-lg font-[620] text-[var(--t1)]">No workspaces yet</p>
        <p class="mx-auto mt-2 max-w-md text-sm leading-7 text-[var(--t4)]">
          Create your first workspace, connect a repo, and start bounded background agents.
        </p>
        <Button
          class="mt-5 h-10 gap-2 rounded-full bg-[var(--accent)] px-4 text-white shadow-none hover:opacity-90"
          onclick={openNewModal}
        >
          <PlusIcon size={15} />
    ...
    {:else}
      <div class="flex flex-col gap-2">
        {#each workspaces as s (s.id)}
          <div
            class="group cursor-pointer rounded-xl border border-[var(--b1)] bg-[var(--bg)] p-4 transition-colors hover:border-[var(--b2)]"
            role="button"
            tabindex="0"
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-medium text-[var(--t1)]">{s.name}</span>
                <span class="rounded-full border border-[var(--b1)] bg-[var(--bg2)] px-2 py-0.5 text-[11px] text-[var(--t4)]">
              ...
              <div class="flex items-center gap-3">
                ...
                {/if}
                <span class="inline-flex items-center justify-center text-[var(--t5)]">
                  <ArrowRightIcon size={14} />
                </span>
              </div>
            </div>
            <div class="mt-2 flex flex-wrap gap-4 pl-4 text-xs text-[var(--t5)]">
    ...
    >
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="w-full max-w-[400px] rounded-2xl border border-[var(--b1)] bg-[var(--bg)] p-6 shadow-[var(--shadow)]" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
        <h2 id="new-workspace-title" class="mb-5 text-lg font-semibold tracking-tight text-[var(--t1)]">New workspace</h2>

        <label class="mb-3.5 block text-sm font-medium text-[var(--t4)]">
          Name
          <input type="text" class="mt-1.5 block w-full rounded-xl border border-[var(--b1)] bg-[var(--bg2)] px-3 py-2.5 text-sm text-[var(--t1)] outline-none focus:border-[var(--accent)]" placeholder="my-project" bind:value={newName} />
        </label>

        <label class="mb-3.5 block text-sm font-medium text-[var(--t4)]">
          Git repo URL <span class="text-[var(--t5)]">(optional)</span>
          <input type="text" class="mt-1.5 block w-full rounded-xl border border-[var(--b1)] bg-[var(--bg2)] px-3 py-2.5 text-sm text-[var(--t1)] outline-none focus:border-[var(--accent)]" placeholder="https://github.com/user/repo" bind:value={newGitUrl} />
        </label>

        <div class="flex justify-end gap-2 mt-5">
          <Button variant="outline" onclick={() => (showNewModal = false)}>Cancel</Button>
          <Button class="bg-[var(--accent)] text-white shadow-none hover:opacity-90" disabled={isCreating} onclick={createWorkspace}>
