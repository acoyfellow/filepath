...
<div class="fixed inset-0 z-[80] flex items-center justify-center bg-black/30 px-3 max-[720px]:items-start max-[720px]:p-2" onclick={(event) => event.target === event.currentTarget && onclose()}>
  <div
    class="max-h-[calc(100vh-24px)] w-full max-w-[760px] overflow-auto rounded-2xl border border-[var(--b1)] bg-[var(--bg)] shadow-[var(--shadow)] max-[720px]:max-h-[calc(100vh-16px)] max-[720px]:rounded-[14px]"
    role="dialog"
    aria-modal="true"
  ...
  >
    <header class="flex items-center justify-between gap-3 px-6 pb-0 pt-5 max-[720px]:px-4 max-[720px]:pt-4">
      <div class="text-xl font-[650] tracking-[-0.02em] text-[var(--t1)] max-[720px]:text-lg" id="create-agent-title">new agent</div>
      <Button variant="outline" size="icon-sm" class="size-8 rounded-full border-[var(--b1)] bg-[var(--bg2)] text-[var(--t3)] shadow-none hover:border-[var(--t4)] hover:bg-[var(--bg3)] hover:text-[var(--t1)]" type="button" onclick={onclose} aria-label="Close create agent modal" title="Close create agent modal">
        <XIcon size={15} />
      </Button>
    </header>
