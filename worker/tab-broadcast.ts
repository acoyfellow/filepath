...
export class TabBroadcastDO extends DurableObject {
  ...
  private async handleWebSocket(request: Request): Promise<Response> {
    ...
    try {
      ...
      console.error('TabBroadcast WebSocket handler error:', error);
  ...
  private async connectTtyd(request: Request, sandboxId: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log('TabBroadcast: Container may be sleeping, attempting wake:', sandboxId);
        try {
          ...
          console.error('TabBroadcast: Wake failed:', wakeError);
